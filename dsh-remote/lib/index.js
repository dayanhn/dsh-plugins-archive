...
export const Config = z.object({
  ...
  /** rw_sync/rw_push parallel file transfers (directory traversal caps at 8). */
  syncConcurrency: z.number().step(1).min(1).max(64).default(16),
...
class SshPool {
  constructor(config) {
    ...
    this.proxyPool = null
    // Cached SFTP subsystem session { client, raw, adapter } — one per pooled
    // SSH connection; replaced by dropSftpSession()/invalidate()/close().
    this.sftpSession = null
    // In-flight sftp() open, shared by concurrent callers.
  ...
  async _doConnect(epoch) {
    ...
    return new Promise((resolve, reject) => {
      ...
      client.on('close', () => {
        if (isCurrent()) this.dropSftpSession()
        if (isCurrent() && this.client === client) this.client = null
   ...
   *
   * The SFTP subsystem session is cached in this.sftpSession — ONE per pooled
   * SSH connection, not one per call. Opening a fresh channel per call and
   * never closing it leaked a session per rw_* tool call until the server's
   * MaxSessions (default 10, tighter on bastions) started killing channels:
   * "Received unexpected SFTP session termination" on a healthy connection.
   *
   * A session the server kills mid-operation is replaced transparently: every
   * adapter op retries once on a freshly acquired session (buildSftpAdapter).
   * A session channel killed mid-establishment ("Received unexpected SFTP
   * session termination") retries once on a fresh connection, like exec().
   * invalidate()/close()/client 'close' drop the cache.
   */
  sftp() {
    if (this.sftpSession) return Promise.resolve(this.sftpSession.adapter)
        ...
        new Promise((resolve, reject) => {
          ...
          const runOn = (c) => {
            ...
            const openErr = (err) => {
              ...
              // "Received unexpected SFTP session termination" is ssh2's
              // onExit for a channel closed mid-establishment (no exit code
              // or signal) — a hop killed the new session channel, usually
              // session-limit pressure; a fresh connection gets a fresh
              // channel budget, so it is retryable here.
              if (!retried && /channel open failure|open failed|unexpected SFTP session termination|connection closed|sftp open timed out/i.test(String((err && err.message) || err))) {
                retried = true
                this.invalidate()
                return this.connect().then(
                  (fresh) => runOn(fresh),
            ...
            const timer = setTimeout(() => {
              if (settled) return
              settled = true
              openErr(new Error('ssh sftp open timed out'))
            }, Math.max(this.config.connectTimeoutMs, 10000))
            c.sftp((err, sftp) => {
              clearTimeout(timer)
              if (settled) {
                // Timed out and the late-opened channel still arrived.
                try { if (sftp) sftp.end() } catch {}
                return
              }
              settled = true
              if (err) return openErr(err)
              const prev = this.sftpSession
              this.sftpSession = { client: c, raw: sftp, adapter: this.buildSftpAdapter(sftp) }
              if (prev) {
                // A parallel open cached first; this one is superseded.
                try { prev.raw.end() } catch {}
              }
              resolve(this.sftpSession.adapter)
            })
          }
          runOn(client)
        }),
    )
    this.sftpOpening = pending
    pending.catch(() => {
      if (this.sftpOpening === pending) this.sftpOpening = null
    })
    return pending
  }

  /**
   * Wrap one raw ssh2 SFTP session in the promisified adapter. Command-style
   * ops share the command budget; transfers get a 2-minute floor — a
   * multi-hop chain can legitimately move megabytes slower than the 20s
   * command budget.
   */
  buildSftpAdapter(raw) {
    const P = (p) => toSftpPath(p)
    const withTimeout = (ms, fn) => (...args) =>
      new Promise((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error('sftp operation timed out')), ms)
        const done = (e, v) => {
          clearTimeout(timer)
          e ? reject(e) : resolve(v)
        }
        try { fn(...args, done) } catch (e) { clearTimeout(timer); reject(e) }
      })
    const cmd = (fn) => withTimeout(this.config.commandTimeoutMs, fn)
    const xfer = (fn) => withTimeout(Math.max(this.config.commandTimeoutMs, 120000), fn)
    const api = {
      readdir: (dir) => cmd((d, cb) => raw.readdir(d, cb))(P(dir)),
      stat: (p) => cmd((d, cb) => raw.stat(d, cb))(P(p)),
      lstat: (p) => cmd((d, cb) => raw.lstat(d, cb))(P(p)),
      mkdir: (dir) => cmd((d, cb) => raw.mkdir(d, cb))(P(dir)),
      rmdir: (dir) => cmd((d, cb) => raw.rmdir(d, cb))(P(dir)),
      unlink: (p) => cmd((d, cb) => raw.unlink(d, cb))(P(p)),
      rename: (p, d) => cmd((a, b, cb) => raw.rename(a, b, cb))(P(p), P(d)),
      realpath: (p) => cmd((d, cb) => raw.realpath(d, cb))(P(p)),
      readFile: (p) => xfer((d, cb) => raw.readFile(d, cb))(P(p)),
      writeFile: (p, data) => xfer((d, data2, cb) => raw.writeFile(d, data2, cb))(P(p), data),
      fastGet: (p, lp) => xfer((d, l, cb) => raw.fastGet(d, l, cb))(P(p), lp),
      fastPut: (p, lp) => xfer((d, l, cb) => raw.fastPut(d, l, cb))(P(p), lp),
    }
    // The server can kill the subsystem session (MaxSessions pressure,
    // bastion idle limits) while the SSH connection itself stays up. Drop
    // the dead session and retry the op ONCE on a freshly acquired one;
    // retried writes re-send full content, so a half-transferred file cannot
    // survive. (A mkdir retried after the original already landed surfaces
    // "already exists" to that one caller — callers that care treat it as
    // success.)
    const terminated = (e) =>
      /unexpected SFTP session termination|SFTP session closed|session was closed|channel (was )?(closed|broken)|EOF|ECONNRESET|connection closed/i.test(
        String((e && e.message) || e),
      )
    const resilient = (op) => (...args) =>
      api[op](...args).catch((e) => {
        if (!terminated(e)) return Promise.reject(e)
        this.dropSftpSession()
        return this.sftp().then((a) => a[op](...args))
      })
    return {
      readdir: resilient('readdir'),
      stat: resilient('stat'),
      lstat: resilient('lstat'),
      mkdir: resilient('mkdir'),
      rmdir: resilient('rmdir'),
      unlink: resilient('unlink'),
      rename: resilient('rename'),
      realpath: resilient('realpath'),
      readFile: resilient('readFile'),
      writeFile: resilient('writeFile'),
      fastGet: resilient('fastGet'),
      fastPut: resilient('fastPut'),
      close: () => this.dropSftpSession(),
    }
  }

  /** Drop the cached SFTP session (the channel may already be dead). */
  dropSftpSession() {
    const s = this.sftpSession
    this.sftpSession = null
    if (s) {
      try { s.raw.end() } catch {}
    }
...
export async function apply(ctx, config) {
  ...
  // Re-apply the persisted active machine to the pool on boot. setCurrent
  // does this for GUI machine switches, but a restart otherwise left the
  // pool on the empty plugin defaults until a manual re-select — the
  // explorer then showed "未设置远程工作区" and browses failed with
  // "no credentials". setTarget is lazy (no SSH connection is made here),
  // so this is harmless when the machine's tunnel is not up yet.
  await applyActiveMachine()
  ...
  const flushAutoPush = async (localDir) => {
    ...
    const next = { ...state }
    if (rels.includes('*')) {
      const r = await pushTree(sftp, localDir, ws, { maxFiles: 500, maxFileBytes: config.maxFileBytes, isIgnored: matcher, state: next, concurrency: config.syncConcurrency })
  ...
  const listDirStructured = async (p) => {
    ...
    // stat() — not lstat(): readdir attrs are lstat semantics (the link
    // itself), so only stat follows the link and can tell dir-target from
    // file-target; a broken link fails stat and degrades to 'file'.
    if (symIdx.length) {
      await Promise.all(symIdx.map(async (i) => {
        ...
        try {
          ...
          items[i].symlink = true
        } catch { /* broken link → stays 'file' */ }
      }))
    }
    ...
    defineTool({
      ...
      async execute(args) {
        ...
        const runSync = async () => {
          ...
          const { stats, nextState } = await syncTree(sftp, p, local, { ...body, state, maxFileBytes: config.maxFileBytes, concurrency: config.syncConcurrency })
          ...
          if (stats.symlinks) text += ` ${stats.symlinks} symlink(s) skipped (not mirrored).`
    ...
    defineTool({
      ...
      async execute(args) {
        ...
        const runPush = async () => {
          ...
          const state = loadSyncState(local)
          const { stats, nextState } = await pushTree(sftp, local, p, { ...body, state, maxFileBytes: config.maxFileBytes, concurrency: config.syncConcurrency })
          if (!args.dryRun) saveSyncState(local, nextState)
  ...
  // ── remote git (source-control tab): git runs on the REMOTE over exec ────
  // Every call: LC_ALL=C (the target's locale is zh — porcelain output must
  // stay stable), --no-pager, -c color.ui=false, shell-quoted paths. `path`
  // params are workspace-root-relative (or absolute); file params come from
  // `git status` (repo-root-relative) and are quoted, never shell-expanded.
  const shq = (s) => `'${String(s).replace(/'/g, `'\\''`)}'`
  const gitRepoPath = (raw) => {
    const rel = String(raw || '').trim()
    if (!rel) throw new Error('git: path required')
    if (rel.startsWith('/')) return normalizeRemotePath(rel)
    if (rel.split('/').some((seg) => seg === '..' || seg === '')) throw new Error('git: invalid path')
    const base = wsPath()
    if (!base) throw new Error('no remote workspace set — pick a workspace first')
    return joinRemotePath(base, rel)
  }
  const runGit = async (repo, args, timeoutMs) => {
    const res = await pool.exec(
      `LC_ALL=C git -C ${shq(gitRepoPath(repo))} -c color.ui=false --no-pager ${args}`,
      { timeoutMs: timeoutMs || Math.max(config.commandTimeoutMs, 60000) },
    )
    if (res.code !== 0) {
      throw new Error('git failed (exit ' + res.code + '): ' + String(res.stderr || res.stdout || '').trim().slice(0, 400))
    }
    return res.stdout
  }
  // `git status --porcelain=v1 -z --branch`: NUL-framed records, the first is
  // the `## ` branch header ("## main...origin/main", "##  HEAD (no branch)",
  // status brackets like "[ahead 2, behind 1]" / "[gone]" allowed). Rename and
  // copy entries carry their origin path as the next record. A truncated tail
  // (exec maxOutputChars) degrades to one bad row, never a crash.
  const parseStatusZ = (out) => {
    const recs = out.split('\0')
    let branch = 'HEAD'
    let upstream = ''
    let detached = true
    let i = 0
    if (recs[0] && recs[0].startsWith('##')) {
      const body = recs[0].slice(2).replace(/^ +/, '')
      if (body.startsWith('HEAD (no branch)')) {
        branch = 'HEAD'
      } else if (body.startsWith('HEAD ')) {
        branch = body.slice(5).trim()
      } else {
        detached = false
        const parts = body.split('...')
        branch = (parts[0] || '').replace(/ \[[^\]]*\]$/, '').trim()
        upstream = (parts[1] || '').replace(/ \[[^\]]*\]$/, '').trim()
      }
      i = 1
    }
    const entries = []
  ...
  // ── open remote files like local ones: full-page browser tab + local app ──
  // /dsh-remote/file streams one remote file to the browser (text sniffed);
  // /dsh-remote/open lands it in a local tmp dir and spawns the OS default
  // app (WPS for .pptx when that is the user's association).
  const REMOTE_FILE_TYPES = {
    '.png': 'image/png',
    '.jpg': 'image/jpeg',
    '.jpeg': 'image/jpeg',
    '.gif': 'image/gif',
  ...
  const requireWorkspaceFile = async (rawPath) => {
    const ws = wsPath()
    if (!ws) return { error: 'no remote workspace set' }
    const p = normalizeRemotePath(String(rawPath || ''))
    if (!p || (p !== ws && !p.startsWith(ws + '/'))) return { error: 'path outside the remote workspace' }
    const sftp = await pool.sftp()
    const st = await sftp.stat(p).catch(() => null)
    if (!st || st.isDirectory()) return { error: 'not a file' }
    const cap = config.maxFileBytes > 0 ? config.maxFileBytes : 50 * 1024 * 1024
  ...
  const routes = [
    ...
    {
      kind: 'exact',
      path: '/dsh-remote/file',
      handler: async (req, res) => {
        if (req.method !== 'GET') return sendJson(res, 405, { error: 'method not allowed' })
        try {
          const q = new URL(req.url, 'http://localhost').searchParams
          const got = await requireWorkspaceFile(q.get('path') ? decodeURIComponent(q.get('path')) : '')
          if (got.error) return sendJson(res, 400, { error: got.error })
          const buf = await got.sftp.readFile(got.p)
          let type = REMOTE_FILE_TYPES[path.extname(got.p).toLowerCase()] ?? 'application/octet-stream'
          // Unknown extension: NUL-free head 8KB is text (code/config/log) and
          // renders full-page in a tab; binaries keep the download behavior.
          if (type === 'application/octet-stream' && buf.length > 0 && !buf.subarray(0, 8192).includes(0)) type = 'text/plain; charset=utf-8'
          res.statusCode = 200
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (got.error) return sendJson(res, 400, { ok: false, error: got.error })
          const dir = path.join(tmpdir(), 'dsh-remote-open', Date.now().toString(36) + '-' + process.pid + '-' + openSeq++)
          mkdirSync(dir, { recursive: true })
          const local = path.join(dir, path.basename(got.p))
          await got.sftp.fastGet(got.p, local)
          launchLocalApp(local)
          audit('open', `open ${got.p} → ${local}`, 0)
          return sendJson(res, 200, { ok: true, local })
        } catch (err) {
          return sendJson(res, 500, { ok: false, error: String((err && err.message) || err) })
        }
      },
    },
    // ── remote git: the source-control tab. `path` = repo relative to the
    // remote workspace root ('.' = the root itself). ─────────────────────────
    {
      kind: 'exact',
      path: '/dsh-remote/git/repos',
      handler: async (req, res) => {
        if (req.method !== 'GET') return sendJson(res, 405, { error: 'method not allowed' })
        try {
          const q = new URL(req.url, 'http://localhost').searchParams
          const base = q.get('path') ? normalizeRemotePath(decodeURIComponent(q.get('path'))) : wsPath()
          if (!base) return sendJson(res, 400, { error: 'no remote workspace set' })
          // .git up to two levels below base (base/repo/.git, base/a/b/.git);
          // node_modules pruned; the workspace root itself counts as '.'.
          // -L: repos are often SYMLINKED dirs (versioned workspaces) and
          // plain find never descends into them; -maxdepth still bounds any
          // symlink cycle.
          const out = await pool.exec(
            `find -L ${shq(base)} -mindepth 1 -maxdepth 3 \\( -name node_modules -prune \\) -o -name .git \\( -type d -o -type f \\) -print 2>/dev/null`,
            { timeoutMs: 20000 },
          )
          ...
          for (const line of lines) {
            const l = line.trim()
            if (!l.endsWith('/.git')) continue
            let rel = l.slice(0, -5)
            if (rel.startsWith(base)) rel = rel.slice(base.length).replace(/^\/+/, '')
            if (!rel || rel.includes('/.git/')) continue
            repos.push(rel)
          }
          if (lines.some((l) => l.trim() === base + '/.git')) repos.unshift('.')
          repos.sort((a, b) => a.localeCompare(b))
          return sendJson(res, 200, { path: base, repos })
        } catch (err) {
          return sendJson(res, 500, { error: String((err && err.message) || err) })
        }
      },
    ...
    {
      kind: 'exact',
      path: '/dsh-remote/git/status',
      handler: async (req, res) => {
        if (req.method !== 'GET') return sendJson(res, 405, { error: 'method not allowed' })
        try {
          const q = new URL(req.url, 'http://localhost').searchParams
          const p = String(q.get('path') || '').trim()
          const parsed = parseStatusZ(await runGit(p, 'status --porcelain=v1 -z --branch'))
          return sendJson(res, 200, { path: p, isRepo: true, branch: parsed.branch, upstream: parsed.upstream, detached: parsed.detached, entries: parsed.entries })
        } catch (err) {
          const msg = String((err && err.message) || err)
          return sendJson(res, /not a git repository/i.test(msg) ? 404 : 500, { error: msg })
        }
      },
    },
    {
      kind: 'exact',
      path: '/dsh-remote/git/diff',
      handler: async (req, res) => {
        if (req.method !== 'GET') return sendJson(res, 405, { error: 'method not allowed' })
        try {
          const q = new URL(req.url, 'http://localhost').searchParams
          const p = String(q.get('path') || '').trim()
          const file = decodeURIComponent(String(q.get('file') || ''))
          const staged = q.get('staged') === '1'
          if (!file) return sendJson(res, 400, { error: 'file required' })
          if (q.get('untracked') === '1') {
            // Untracked files have no diff: serve the content itself (capped),
            // binary-detected via NUL probe — mirrors /dsh-remote/read.
            const abs = joinRemotePath(gitRepoPath(p), file)
            const s = await pool.sftp()
            const st = await s.stat(abs).catch(() => null)
            if (!st) return sendJson(res, 404, { error: 'not found: ' + file })
            const cap = 256 * 1024
            if (st.size > cap) return sendJson(res, 200, { untracked: true, binary: true, size: st.size, truncated: true })
            const buf = await s.readFile(abs)
            const b = Buffer.isBuffer(buf) ? buf : Buffer.from(buf)
            const binary = b.includes(0)
            return sendJson(res, 200, { untracked: true, binary, size: st.size, content: binary ? '' : b.toString('utf8') })
          }
          const out = await runGit(p, `diff ${staged ? '--cached ' : ''}-- ${shq(file)}`)
          return sendJson(res, 200, { diff: out })
        } catch (err) {
          return sendJson(res, 500, { error: String((err && err.message) || err) })
        }
      },
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          await runGit(p, file ? `add -A -- ${shq(file)}` : 'add -A')
          audit('git', `git stage ${p}${file ? ' ' + file : ''}`, 0)
          return sendJson(res, 200, { ok: true })
        } catch (err) {
          audit('git', `git stage ${String((body && body.path) || '')}`, 1)
          return sendJson(res, 500, { ok: false, error: String((err && err.message) || err) })
        }
      },
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const p = String(body.path || '').trim()
          const file = String(body.file || '').trim()
          await runGit(p, file ? `reset -q -- ${shq(file)}` : 'reset -q')
          audit('git', `git unstage ${p}${file ? ' ' + file : ''}`, 0)
          return sendJson(res, 200, { ok: true })
        } catch (err) {
          audit('git', `git unstage ${String((body && body.path) || '')}`, 1)
          return sendJson(res, 500, { ok: false, error: String((err && err.message) || err) })
        }
      },
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (!message) return sendJson(res, 400, { ok: false, error: 'commit message required' })
          await runGit(p, `commit -m ${shq(message)}`)
          audit('git', `git commit ${p}: ${message.slice(0, 60)}`, 0)
          return sendJson(res, 200, { ok: true })
        } catch (err) {
          const msg = String((err && err.message) || err)
          audit('git', `git commit ${String((body && body.path) || '')}`, 1)
          // "nothing to commit" is a normal state, not a failure.
          return sendJson(res, /nothing to commit|no changes added/i.test(msg) ? 409 : 500, { ok: false, error: msg })
        }
      },
    },
    {
      kind: 'exact',
      path: '/dsh-remote/git/branches',
      handler: async (req, res) => {
        if (req.method !== 'GET') return sendJson(res, 405, { error: 'method not allowed' })
        try {
          const q = new URL(req.url, 'http://localhost').searchParams
          const p = String(q.get('path') || '').trim()
          const [namesOut, curOut] = await Promise.all([
            // %(…): the ( ) are shell metacharacters — the format value must
            // be quoted for the remote bash.
            runGit(p, `for-each-ref --format=${shq('%(refname:short)')} refs/heads/`),
            runGit(p, 'rev-parse --abbrev-ref HEAD'),
          ])
          const names = namesOut.split('\n').map((l) => l.trim()).filter((l) => l)
          return sendJson(res, 200, { current: curOut.trim() || 'HEAD', names })
        } catch (err) {
          return sendJson(res, 500, { error: String((err && err.message) || err) })
        }
      },
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (!branch) return sendJson(res, 400, { ok: false, error: 'branch required' })
          await runGit(p, `checkout -q ${shq(branch)}`)
          audit('git', `git checkout ${p} ${branch}`, 0)
          return sendJson(res, 200, { ok: true })
        } catch (err) {
          audit('git', `git checkout ${String((body && body.path) || '')}`, 1)
          return sendJson(res, 500, { ok: false, error: String((err && err.message) || err) })
        }
      },
    },
    {
      kind: 'exact',
      path: '/dsh-remote/git/commit-diff',
      handler: async (req, res) => {
        if (req.method !== 'GET') return sendJson(res, 405, { error: 'method not allowed' })
        try {
          const q = new URL(req.url, 'http://localhost').searchParams
          ...
          if (!/^[0-9a-f]{4,40}$/i.test(hash)) return sendJson(res, 400, { error: 'bad hash' })
          // show --pretty=format: → patch only; root commits diff vs the empty
          // tree on modern git (the target runs 2.33).
          const out = await runGit(p, `show --pretty=format: ${shq(hash)}`)
          return sendJson(res, 200, { diff: out })
        } catch (err) {
          return sendJson(res, 500, { error: String((err && err.message) || err) })
        }
    ...
    {
      kind: 'exact',
      path: '/dsh-remote/git/log',
      handler: async (req, res) => {
        if (req.method !== 'GET') return sendJson(res, 405, { error: 'method not allowed' })
        try {
          const q = new URL(req.url, 'http://localhost').searchParams
          const p = String(q.get('path') || '').trim()
          const count = Math.min(Math.max(parseInt(q.get('count') || '30', 10) || 30, 1), 200)
          const skip = Math.max(parseInt(q.get('skip') || '0', 10) || 0, 0)
          const out = await runGit(p, `log --pretty=format:%h%x1f%H%x1f%s%x1f%an%x1f%ai%x1f%D --max-count=${count} --skip=${skip}`)
          const entries = []
          for (const line of out.split('\n')) {
            if (!line) continue
            const [hash, hashFull, subject, author, date, refs = ''] = line.split('\x1f')
            if (!hash) continue
            entries.push({ hash, hashFull, subject, author, date, refs })
          }
          return sendJson(res, 200, { entries })
        } catch (err) {
          const msg = String((err && err.message) || err)
          // A repo without commits is an empty state, not an error.
          if (/does not have any commits|empty repository/i.test(msg)) return sendJson(res, 200, { entries: [] })

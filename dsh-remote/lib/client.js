...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SIDEBAR_GIT_ID = 'dsh-remote:git'
    ...
    function RemoteExplorerTab(props) {
          ...
          if (r) {
            ...
            listRemoteDir(r)
              .then((res) => {
                ...
                // Re-expand only dirs under THIS root: expansions persist across
                // workspace switches and stale absolute paths 500 on the new one.
                for (const dir of expanded) {
                  if (dir === r || String(dir).startsWith(r + '/')) loadDir(dir)
                }
              })
      ...
      // Load dirs as they get expanded (only ones under the current root —
      // see the replay filter in refreshStatus).
      React.useEffect(() => {
        if (!root) return
        for (const dir of expanded) {
          if (dir === root || String(dir).startsWith(root + '/')) loadDir(dir)
        }
      }, [root, expanded, loadDir])
      React.useEffect(() => {
        if (!menu) return
        // Close only on mousedowns that START outside the menu: the item's own
        // click arrives as mousedown→mouseup, and a naive document listener
        // unmounts the menu on the first half of that gesture, so no item
      ...
      // Path as the agent sees it: relative to this session's remote workspace
      // root (the tree is rooted there), '.' for the root itself (same
      // convention as dsh-better-sidebar's relativeTo), full path fallback.
      const relOf = (p) => {
        const s = String(p)
        if (root && s === root) return '.'
        return root && s.indexOf(root + '/') === 0 ? s.slice(root.length + 1) : s
      }

      // Append @<path> to this session's conversation draft — the same
      // mechanism as dsh-better-sidebar's file tab @-reference
      // (sessions.scope → conversation.input.for → setDraft).
      const referenceInChat = (p) => {
        const ctx = props.ctx
        if (!ctx || !sessionId) return
        const sessions = ctx.sessions || (typeof ctx.get === 'function' && ctx.get('sessions'))
        ...
        if (!actx || !conversation || !conversation.input) {
          console.warn('[dsh-remote] @-reference skipped: conversation input unavailable')
          return
        }
        try {
          const input = conversation.input.for(actx)
          const draft = input.state.getSnapshot().draft
          input.setDraft(draft.trim() === '' ? '@' + relOf(p) : draft + ' @' + relOf(p))
        } catch (e) {
          console.warn('[dsh-remote] @-reference failed:', e)
        }
      }

      // The hover-revealed pill button at a row's far right (files AND
      // folders): references the entry into the conversation draft.
      const atButton = (p) => React.createElement('button', {
        type: 'button',
        onClick: (e) => { e.stopPropagation(); referenceInChat(p) },
        onMouseDown: (e) => e.stopPropagation(),
        title: '引用到对话输入框（@' + relOf(p) + '）',
        style: { flexShrink: 0, height: 20, padding: '0 8px', border: '1px solid ' + T.border, borderRadius: 999, background: T.bg2, color: T.label, fontSize: 11, lineHeight: '18px', cursor: 'pointer' },
      }, '@')
      ...
      const ctxItem = (label, danger, fn) => React.createElement('div', {
        ...
        // Theme tokens (not hardcoded light colors): the menu follows the
        // dsw light/dark alias like the rest of the tab.
        ...
        if (entry.type === 'dir') {
          ...
          return React.createElement('div', { key: entry.path },
            React.createElement('div', {
              ...
              hovered ? atButton(entry.path) : null,
            ),
          ...
          !root
            ...
            : React.createElement('div', { key: root },
                React.createElement('div', {
                  ...
                  onMouseEnter: () => setHoverPath(root),
                  onMouseLeave: () => setHoverPath((p) => (p === root ? '' : p)),
                  ...
                  hoverPath === root ? atButton(root) : null,
                ),
                rootLevel && rootOpen ? renderLevel(root, 1) : null,
              ),
        ),
        // context menu — the ref callback re-anchors menu.x/menu.y into the
        // effective fixed-position containing block (a sidebar pane carries
        // CSS `contain`, which would otherwise shift the menu off-screen).
        menu ? React.createElement('div', {
          ref: (el) => {
            menuRef.current = el
            // The menu is position:fixed, but the FIRST ancestor that carries
            // a transform/filter/contain becomes the fixed containing block
            // (the sidebar panes use `contain`) — re-anchor the viewport
            // cursor coords into that element's space, or the menu lands
            // off-screen and right-click looks dead.
            if (el && menu) {
              let n = el.parentElement
              while (n && n !== document.documentElement) {
                const s = getComputedStyle(n)
                if (s.transform !== 'none' || s.filter !== 'none' || s.perspective !== 'none' || s.contain !== 'none' || s.backdropFilter !== 'none') {
                  const r = n.getBoundingClientRect()
                  el.style.left = (menu.x - r.left) + 'px'
                  el.style.top = (menu.y - r.top) + 'px'
                  break
                }
                n = n.parentElement
              }
          ...
          style: { position: 'fixed', left: menu.x, top: menu.y, zIndex: 9998, background: v('--dsw-alias-bg-overlay', '#1f1f23'), border: '1px solid ' + T.borderStrong, borderRadius: 8, padding: 4, boxShadow: '0 8px 24px rgba(0,0,0,0.4)', minWidth: 170 },
          ...
          !menu.isDir ? ctxItem('🌐 在浏览器新标签页打开', false, () => {
            const w = window.open('/dsh-remote/file?path=' + encodeURIComponent(menu.path), '_blank')
            if (!w) setErr('浏览器拦截了新标签页：请在地址栏允许本站弹窗后重试')
          }) : null,
          !menu.isDir ? ctxItem('↗ 用外部应用打开（Pptx 走 WPS）', false, () => {
            // SFTP 下载到本地 tmp 后由 host 拉起系统默认应用；原文件不动。
            api('POST', '/dsh-remote/open', { path: menu.path }).then((d) => {
              if (d && d.ok) setErr('')
            }).catch((e) => setErr(String((e && e.message) || e)))
          }) : null,
    ...
    // ── Remote source-control tab (git runs on the REMOTE over SSH) ────────
    // Discovers the git repos nested in the remote workspace (symlinked dirs
    // included) and drives status / stage / commit / branch / log / diff
    // against the selected one. Pure view over /dsh-remote/git/*; the agent's
    // rw_* tools remain the canonical interface for file work.
    const GIT_LOG_BATCH = 20
    const gitStaged = (xy) => !!xy && xy[0] !== ' ' && xy[0] !== '?'
    const gitUnstaged = (xy) => !!xy && (xy === '??' || (xy[1] !== ' ' && xy[1] !== '?'))

    function RemoteGitTab(props) {
      const [repos, setRepos] = React.useState([])
      const [repo, setRepo] = React.useState('')
      const [st, setSt] = React.useState(null)           // {branch, upstream, detached, entries}
      const [branches, setBranches] = React.useState(null)
      const [log, setLog] = React.useState([])
      const [logEnded, setLogEnded] = React.useState(false)
      const [logMore, setLogMore] = React.useState(false)
      const [commitMsg, setCommitMsg] = React.useState('')
      const [err, setErr] = React.useState('')
      const [busy, setBusy] = React.useState('')
      const [loading, setLoading] = React.useState(true)
      const [diff, setDiff] = React.useState(null)       // {title, diff?|content?, binary?, size?}

      const qOf = (r, p) => '/dsh-remote/git/' + p + '?path=' + encodeURIComponent(r)

      const loadRepo = React.useCallback(async (r) => {
        if (!r) return
        setLoading(true)
        setErr('')
        setDiff(null)
        ...
        try {
          const [s, b, l] = await Promise.all([
            api('GET', qOf(r, 'status')),
            api('GET', qOf(r, 'branches')).catch(() => ({ current: '', names: [] })),
            api('GET', qOf(r, 'log') + '&count=' + GIT_LOG_BATCH).catch(() => ({ entries: [] })),
          ])
          setSt(s)
          setBranches(b)
          setLog(l.entries || [])
          setLogEnded((l.entries || []).length < GIT_LOG_BATCH)
        } catch (e) {
      ...
      const loadRepos = React.useCallback(async () => {
        try {
          const r = await api('GET', '/dsh-remote/git/repos')
          const list = r.repos || []
          setRepos(list)
          setRepo((cur) => (list.includes(cur) ? cur : (list[0] || '')))
        } catch (e) {
          setRepos([])
          setRepo('')
          setErr(String((e && e.message) || e))
      ...
      React.useEffect(() => { if (repo) loadRepo(repo) }, [repo, loadRepo])
      React.useEffect(() => { if (props.visible && repo) loadRepo(repo) }, [props.visible])

      const act = (op, payload) => {
        setBusy(op)
        setErr('')
        return api('POST', '/dsh-remote/git/' + op, { path: repo, ...payload })
          .then(() => loadRepo(repo))
          .catch((e) => { setErr(String((e && e.message) || e)); loadRepo(repo) })
          .finally(() => setBusy(''))
      }
      const stage = (file) => act('stage', file ? { file } : {})
      const unstage = (file) => act('unstage', file ? { file } : {})
      const commit = () => {
        const m = commitMsg.trim()
        if (!m || busy) return
        setBusy('commit')
        setErr('')
        api('POST', '/dsh-remote/git/commit', { path: repo, message: m })
          .then(() => { setCommitMsg(''); loadRepo(repo) })
          .catch((e) => setErr(String((e && e.message) || e)))
          .finally(() => setBusy(''))
      }
      const checkout = (b) => {
        if (!b || busy || (st && b === st.branch)) return
        act('checkout', { branch: b })
      }
      const loadMoreLog = () => {
        if (logMore || logEnded) return
        setLogMore(true)
        api('GET', qOf(repo, 'log') + '&count=' + GIT_LOG_BATCH + '&skip=' + log.length)
          .then((r) => {
            const entries = r.entries || []
            setLog((l) => [...l, ...entries])
            if (entries.length < GIT_LOG_BATCH) setLogEnded(true)
          })
          .catch((e) => setErr(String((e && e.message) || e)))
          .finally(() => setLogMore(false))
      }
      const openDiff = (entry, stagedSide) => {
        if (busy) return
        setBusy('diff')
        setErr('')
        const untracked = entry.xy === '??'
        const url = qOf(repo, 'diff') + '&file=' + encodeURIComponent(entry.path)
          + (untracked ? '&untracked=1' : (stagedSide ? '&staged=1' : ''))
        api('GET', url)
          .then((r) => {
            if (r.untracked) setDiff({ title: entry.path + '（新文件）', binary: r.binary, size: r.size, content: r.content || '' })
            else setDiff({ title: entry.path + (stagedSide ? '（已暂存）' : ''), diff: r.diff || '（无差异 — 可能只是权限/换行变化）' })
          })
          .catch((e) => setErr(String((e && e.message) || e)))
          .finally(() => setBusy(''))
      }
      const openCommitDiff = (e) => {
        if (busy) return
        setBusy('diff')
        setErr('')
        api('GET', qOf(repo, 'commit-diff') + '&hash=' + encodeURIComponent(e.hashFull))
          .then((r) => setDiff({ title: e.hash + ' ' + e.subject, diff: r.diff || '' }))
          .catch((er) => setErr(String((er && er.message) || er)))
          .finally(() => setBusy(''))
      }

      const renderDiffText = (text) => (text || '').split('\n').map((ln, i) => {
        let c = 'ctx'
        if (ln.startsWith('diff ') || ln.startsWith('index ') || ln.startsWith('@@')) c = 'meta'
        else if (ln.startsWith('---') || ln.startsWith('+++')) c = 'meta'
        ...
        return React.createElement('div', {
          ...
          style: {
            ...
            color: c === 'add' ? T.ok : c === 'del' ? T.dangerText : T.label,
            opacity: c === 'meta' ? 0.55 : 1,
            background: c === 'add' ? 'rgba(76,175,125,0.08)' : c === 'del' ? 'rgba(224,108,117,0.08)' : 'transparent',
          },
        }, ln || ' ')
      })

      const entryRow = (entry, stagedSide) => React.createElement('div', {
        key: (stagedSide ? 's' : 'u') + ':' + entry.path,
        style: { display: 'flex', alignItems: 'center', gap: 6, padding: '4px 6px', borderRadius: 4, cursor: 'pointer', fontSize: 12.5 },
        onClick: () => openDiff(entry, stagedSide),
        title: entry.path + '（点击看 diff）',
      },
        React.createElement('span', { style: { width: 20, textAlign: 'center', fontWeight: 700, fontSize: 11, flexShrink: 0, color: entry.xy === '??' ? T.warn : stagedSide ? T.ok : T.dangerText } }, entry.xy.trim()),
        React.createElement('span', { style: { flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Consolas, monospace' } }, entry.path),
        React.createElement('button', {
          type: 'button',
          style: { ...buttonS, padding: '2px 8px', fontSize: 11, flexShrink: 0 },
          disabled: !!busy,
          onClick: (e) => { e.stopPropagation(); (stagedSide ? unstage : stage)(entry.path) },
          title: stagedSide ? '取消暂存' : '暂存',
        }, stagedSide ? '取消暂存' : '暂存'),
      )

      const stagedEntries = (st && st.entries || []).filter((e) => gitStaged(e.xy))
      const unstagedEntries = (st && st.entries || []).filter((e) => gitUnstaged(e.xy))
      const curBranch = (st && st.branch) || (branches && branches.current) || ''

      return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, fontSize: 13, color: T.label } },
        // repo picker + refresh
        React.createElement('div', { style: { display: 'flex', gap: 6, alignItems: 'center', padding: '6px 8px', borderBottom: '1px solid rgba(128,128,128,0.25)' } },
          React.createElement('select', {
            ...
            title: '远程工作区下的 git 仓库（含软链目录，两层深度）',
          }, repos.map((r) => React.createElement('option', { key: r, value: r }, r === '.' ? '（工作区根目录）' : r))),
          React.createElement('button', { style: { ...buttonS, padding: '3px 8px', fontSize: 12 }, onClick: () => { loadRepos(); if (repo) loadRepo(repo) }, title: '刷新' }, busy ? '…' : '↻'),
        ),
        // branch picker + upstream
        repo ? React.createElement('div', { style: { display: 'flex', gap: 6, alignItems: 'center', padding: '4px 8px' } },
          React.createElement('select', {
            style: { ...inputS, flex: 1, minWidth: 0 },
            value: curBranch,
            onChange: (e) => checkout(e.target.value),
            disabled: !branches || !branches.names.length || (st && st.detached) || !!busy,
            title: st && st.detached ? '分离 HEAD（无本地分支可切）' : '切换分支',
          },
            curBranch ? React.createElement('option', { value: curBranch }, curBranch + (st && st.detached ? '（分离）' : '')) : null,
            (branches ? branches.names : []).filter((n) => n !== curBranch).map((n) => React.createElement('option', { key: n, value: n }, n))),
          st && st.upstream ? React.createElement('span', { style: { fontSize: 11, opacity: 0.6, flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 140 } }, st.upstream) : null,
        ) : null,
        // body
        React.createElement('div', { style: { flex: 1, overflowY: 'auto', minHeight: 0, padding: '4px 6px' } },
          loading ? React.createElement('div', { style: { opacity: 0.6, padding: 10 } }, '加载中…') : null,
          !loading && err ? React.createElement('div', { style: { color: T.dangerText, fontSize: 12, whiteSpace: 'pre-wrap', wordBreak: 'break-all', padding: '4px 2px' } }, err) : null,
          !loading && !err && !repo ? React.createElement('div', { style: { opacity: 0.6, padding: 10, fontSize: 12.5, lineHeight: 1.6 } }, '远程工作区下未找到 git 仓库（两层深度，含软链目录）。未设置工作区请先在「远程文件」里选择。') : null,
          st && React.createElement(React.Fragment, null,
            React.createElement('div', { style: { display: 'flex', alignItems: 'baseline', gap: 8, padding: '4px 4px 2px' } },
              React.createElement('span', { style: { fontSize: 11, opacity: 0.6 } }, '已暂存 (' + stagedEntries.length + ')'),
              stagedEntries.length ? React.createElement('button', { type: 'button', style: { ...buttonS, padding: '1px 8px', fontSize: 11 }, disabled: !!busy, onClick: () => unstage() }, '全部取消') : null),
            stagedEntries.length ? stagedEntries.map((e) => entryRow(e, true))
              : React.createElement('div', { style: { fontSize: 12, opacity: 0.5, padding: '2px 6px' } }, '（无）'),
            React.createElement('div', { style: { display: 'flex', alignItems: 'baseline', gap: 8, padding: '8px 4px 2px' } },
              React.createElement('span', { style: { fontSize: 11, opacity: 0.6 } }, '未暂存 (' + unstagedEntries.length + ')'),
              unstagedEntries.length ? React.createElement('button', { type: 'button', style: { ...buttonS, padding: '1px 8px', fontSize: 11 }, disabled: !!busy, onClick: () => stage() }, '全部暂存') : null),
            unstagedEntries.length ? unstagedEntries.map((e) => entryRow(e, false))
              : React.createElement('div', { style: { fontSize: 12, opacity: 0.5, padding: '2px 6px' } }, '（无）'),
            React.createElement('div', { style: { display: 'flex', gap: 6, padding: '8px 2px' } },
              React.createElement('input', {
                style: { ...inputS, flex: 1, fontSize: 12.5 },
                placeholder: '提交信息（Ctrl+Enter）',
                value: commitMsg,
                disabled: !!busy,
                onChange: (e) => { setCommitMsg(e.target.value); setErr('') },
                onKeyDown: (e) => { if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') commit() },
              }),
              React.createElement('button', {
                type: 'button',
                style: { ...buttonS, padding: '6px 10px', fontSize: 12.5 },
                disabled: !!busy || !commitMsg.trim() || !stagedEntries.length,
                onClick: commit,
                title: stagedEntries.length ? '提交已暂存的更改' : '先暂存至少一个文件',
              }, '提交')),
            React.createElement('div', { style: { fontSize: 11, opacity: 0.6, padding: '6px 4px 2px' } }, '历史（点击查看完整改动）'),
            log.length ? log.map((e) => React.createElement('div', {
              key: e.hashFull,
              style: { padding: '4px 6px', borderRadius: 4, cursor: 'pointer', fontSize: 12 },
              onClick: () => openCommitDiff(e),
              title: e.author + ' · ' + e.date,
            },
              React.createElement('div', { style: { display: 'flex', gap: 6, overflow: 'hidden' } },
                React.createElement('span', { style: { fontFamily: 'ui-monospace, Menlo, Consolas, monospace', color: T.warn, flexShrink: 0 } }, e.hash),
                React.createElement('span', { style: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, e.subject)),
              React.createElement('div', { style: { fontSize: 11, opacity: 0.55, marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } },
                (e.refs ? e.refs + ' · ' : '') + e.author + ' · ' + e.date),
            )) : React.createElement('div', { style: { fontSize: 12, opacity: 0.5, padding: '2px 6px' } }, '（无提交）'),
            !logEnded ? React.createElement('button', { type: 'button', style: { ...buttonS, padding: '4px 10px', fontSize: 12, marginTop: 4 }, disabled: logMore || !!busy, onClick: loadMoreLog }, logMore ? '加载中…' : '加载更多') : null,
          ),
          diff ? React.createElement('div', { style: { borderTop: '1px solid ' + T.border, marginTop: 10, maxHeight: '45%', overflowY: 'auto', padding: '6px 8px', background: 'rgba(0,0,0,0.15)', borderRadius: 6 } },
            React.createElement('div', { style: { display: 'flex', gap: 6, alignItems: 'center', marginBottom: 6 } },
              React.createElement('span', { style: { flex: 1, fontSize: 12, fontFamily: 'ui-monospace, Menlo, Consolas, monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }, title: diff.title }, diff.title),
              React.createElement('button', { type: 'button', style: { ...buttonS, padding: '2px 8px', fontSize: 11, flexShrink: 0 }, onClick: () => setDiff(null) }, '✕ 关闭')),
            diff.binary
              ? React.createElement('div', { style: { fontSize: 12, color: T.warn } }, '二进制文件（' + (diff.size != null ? diff.size + ' bytes' : '未知大小') + '）— 请在「远程文件」里下载查看。')
              : diff.content !== undefined
                ? React.createElement('div', { style: { fontSize: 12, fontFamily: 'ui-monospace, Menlo, Consolas, monospace', whiteSpace: 'pre-wrap', wordBreak: 'break-all', opacity: 0.9 } }, diff.content)
                : renderDiffText(diff.diff),
          ) : null,
        ),
      )
    }
    ...
    function registerSidebarIntegration(ctx) {
      ...
      try {
        ...
        disposers.push(bs.registerTab({
          ...
          order: 56,
          single: true,
          component: (props) => React.createElement(RemoteGitTab, props),
        }))
      } catch (e) {

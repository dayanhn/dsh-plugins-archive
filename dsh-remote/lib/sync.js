...
import { readdirSync, statSync, lstatSync, mkdirSync, existsSync, readFileSync, writeFileSync, utimesSync, renameSync, unlinkSync } from 'node:fs'
...
export async function syncTree(sftp, remoteDir, localDir, opts = {}) {
  ...
  const stats = { files: 0, dirs: 0, skippedUnchanged: 0, skippedLarge: 0, conflicts: [], staleRemote: 0, touched: [], symlinks: 0 }
  ...
  const pullFile = async (rDir, lDir, e) => {
    ...
    // Never mirror links: fastGet follows them (a dir-target errors, a
    // file-target silently materializes the target's bytes into the mirror,
    // and a later push through that path would clobber the link's target).
    // Symlink attrs come straight from the readdir listing — no extra stat.
    if (e.attrs && e.attrs.isSymbolicLink && e.attrs.isSymbolicLink()) { stats.symlinks++; return }
...
export async function pushTree(sftp, localDir, remoteDir, opts = {}) {
  ...
  const stats = { files: 0, dirs: 0, skippedUnchanged: 0, skippedLarge: 0, conflicts: [], staleLocal: 0, pushed: [], symlinks: 0 }
  const nextState = { ...state }
  let inFlight = 0
  ...
  const pushFile = async (lDir, rDir, e) => {
    ...
    // Never push through a local link: fastPut would follow it and write the
    // remote file at the link's TARGET path. lstatSync — statSync follows the
    // link, so its result is never "symbolic".
    try {
      if (lstatSync(lp).isSymbolicLink()) { stats.symlinks++; return }
    } catch { /* raced away; the stat above is stale but harmless here */ }
...
export async function pushOneFile(sftp, localDir, remoteDir, rel, opts = {}) {
  ...
  // lstatSync — statSync follows the link, so it can never report a link.
  try {
    if (lstatSync(lp).isSymbolicLink()) return { status: 'skipped', reason: 'symlink' }
  } catch { /* raced away; treated as the file statSync saw */ }

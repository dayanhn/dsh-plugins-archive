...
import { chmodSync, existsSync, readdirSync, readFileSync, realpathSync, statSync } from "node:fs";
...
/** Git repos nested below a NON-repo root: subdirectories one or two levels
 *  down that contain a `.git` entry (directory or file — worktrees/submodules
 *  use a file). Symlinked directories are followed (versioned workspaces link
 *  their repo dirs); hidden directories, `.git` and `node_modules` are
 *  skipped. Bounded: at most 2000 directories are visited, so a huge tree can
 *  never stall the status poll. Synchronous on purpose — the walk is local
 *  disk only and the status route already tolerates a few ms. */
function discoverNestedRepos(root) {
	...
	const visit = (dir, rel, depth) => {
		...
		try {
			if (rel !== "" && existsSync(join(dir, ".git"))) found.push(rel);
		} catch {}
		if (depth >= 2) return;
		let entries;
		try {
		...
		for (const entry of entries) {
			...
			visit(join(dir, name), rel === "" ? name : `${rel}/${name}`, depth + 1);
		}
	};
	visit(root, "", 0);
	found.sort();
...
/** Working-tree status (untracked included). When `cwd` is not a repo, also
 *  reports the git repos nested one or two levels below it (`nestedRepos`,
 *  cwd-relative paths) so the git panel can offer them for selection —
 *  workspace roots that CONTAIN several repos otherwise dead-end at
 *  "not a git repository". */
async function status(cwd) {
	if (!await isGitRepo(cwd)) return {
		isRepo: false,
		entries: [],
		nestedRepos: discoverNestedRepos(cwd)
	};
...
/**
* Launch the OS default application for a file and return the detached child:
* `xdg-open` on Linux (follows file associations — e.g. WPS for .pptx when WPS
* is the default handler), `open` on macOS, `start` on Windows. The child is
* unref'd by the caller; the route reports success when the OPENER is spawned,
* not when the app exits.
*/
function spawnDesktopOpen(path) {
	if (process.platform === "win32") return spawn("cmd", ["/c", "start", "", path], {
		detached: true,
		stdio: "ignore"
	});
	if (process.platform === "darwin") return spawn("open", [path], {
		detached: true,
		stdio: "ignore"
	});
	if (spawnSync("which", ["xdg-open"]).status !== 0) throw new SidebarError("fs-error", "no desktop opener found (install xdg-utils for xdg-open)", 400);
	return spawn("xdg-open", [path], {
		detached: true,
		stdio: "ignore"
	});
}
/** Office formats the media route converts to PDF on demand (the browser
 *  has no native renderer for them; LibreOffice does). */
const OFFICE_EXTS = new Set([".pptx", ".ppt", ".docx", ".doc", ".xlsx", ".xls", ".odp", ".ods", ".odt", ".pptm", ".docm", ".xlsm"]);
/** Source files above this are not worth a headless conversion round-trip. */
const OFFICE_SOURCE_LIMIT = 300 * 1024 * 1024;
const OFFICE_CACHE_DIR = join(tmpdir(), "dsh-sidebar-office-cache");
let officeQueue = Promise.resolve();
/**
* Convert one Office file to PDF via headless LibreOffice, memoized by
* path+mtime+size. Conversions serialize on a promise chain (soffice is
* heavy and concurrent instances fight over profile locks), each with its
* own UserInstallation profile dir. Returns the cached PDF path.
*/
async function convertOfficeToPdf(abs) {
	const run = async () => {
		const st = await stat(abs);
		const key = createHash("sha1").update(`${abs}|${st.mtimeMs}|${st.size}`).digest("hex");
		const hit = join(OFFICE_CACHE_DIR, `${key}.pdf`);
		if (existsSync(hit)) return hit;
		const outdir = join(OFFICE_CACHE_DIR, key);
		...
		await new Promise((resolveP, rejectP) => {
			const child = spawn("soffice", [
				...
				"--outdir",
				outdir,
				`-env:UserInstallation=file://${join(outdir, "profile")}`,
				abs
			...
			const timer = setTimeout(() => {
				...
				rejectP(new SidebarError("fs-error", "office conversion timed out", 500));
			}, 180000);
			child.on("error", (e) => {
				clearTimeout(timer);
				rejectP(new SidebarError("fs-error", `cannot run LibreOffice (soffice): ${e instanceof Error ? e.message : String(e)}`, 400));
			});
			child.on("close", (code) => {
				clearTimeout(timer);
				code === 0 ? resolveP() : rejectP(new SidebarError("fs-error", `office conversion failed (exit ${code}): ${stderr.slice(-300)}`, 500));
			});
			child.stderr.on("data", (d) => {
				stderr += String(d);
			});
		});
		const produced = join(outdir, `${basename(abs, extname(abs))}.pdf`);
		if (!existsSync(produced)) throw new SidebarError("fs-error", "office conversion produced no PDF", 500);
		await rename(produced, hit);
		return hit;
	};
	const queued = officeQueue.then(run, run);
	officeQueue = queued.catch(() => {});
	return queued;
}
/**
...
function buildApi(ctx, ptyManager, agentPtyRegistry, resolved, terminalShell, getSettings) {
	const cwdOf = (payload) => {
		...
		// Nested-repo override (git panel): `repoPath` is a cwd-relative
		// segment list resolved under the session cwd — it must stay inside
		// it (no absolute paths, no `..`), mirroring the fs.search containment
		// rule. Only git.* methods ever send it.
		const repoPath = typeof record?.repoPath === "string" ? record.repoPath.trim() : "";
		if (repoPath !== "") {
			const segs = repoPath.split("/");
			if (!repoPath.startsWith("/") && segs.every((s) => s !== "" && s !== ".." && s !== ".")) {
				const candidate = resolve(base.cwd, repoPath);
				if (candidate.startsWith(base.cwd + sep)) return { sessionId, cwd: candidate };
			}
		}
		return base;
	};
	...
	return {
		...
		/** Open a file in the OS default application (host-side launch — the
		*  browser itself cannot start desktop apps). Path must stay inside the
		*  session working directory, same fence as the media route. */
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (isOffice && !serveRaw) {
					...
					const pdfBody = await readFile(await convertOfficeToPdf(path));
					res.writeHead(200, {
				...
				let type = mediaTypeForPath(path);
				const body = await readFile(path);
				// Unknown extension: sniff the head instead of forcing a
				// download — NUL-free content is text (code/config/log) and
				// renders full-page in a browser tab; NUL present keeps the
				// octet-stream download behavior.
				if (type === "application/octet-stream" && body.length > 0 && !body.subarray(0, 8192).includes(0)) type = "text/plain; charset=utf-8";
				const headers = {
					"content-type": type,
					"cache-control": "no-cache"
				};

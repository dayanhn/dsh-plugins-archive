	...
	factory: (require) => {
		...
		*/
		...
		const en = {
			...
			nestedRepos: "Repositories in subdirectories:",
			backToRoot: "Back to directory",
		...
		*/
		...
		/** Fold a scope into a JSON payload ({cwd} only when present; plus the
		*  nested-repo override {repoPath} when the git panel selected one). */
		function scopePayload(scope, extra) {
			return {
				sessionId: scope.sessionId,
				...scope.cwd !== void 0 && scope.cwd !== "" ? { cwd: scope.cwd } : {},
				...scope.repoPath !== void 0 && scope.repoPath !== "" ? { repoPath: scope.repoPath } : {},
				...extra
			};
		}
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [root === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					onSelect: (id) => {
						...
						if (id === "open-browser") {
							// Full-page view: the media route serves text-like
							// files as text/plain (sniffed), images/PDF natively,
							// Office as a converted PDF; unknown binaries download
							// — same as 下载. A blocked popup (null) falls back to
							// the sidebar tab so the content is still reachable.
							const win = window.open(fileUrl({
								sessionId,
								cwd
							}, target.path, false), "_blank");
							if (!win) {
								onOpenFileNewTab?.(target.path);
		...
		*  floods the panel at once (the end of the log is reached by paging). */
		...
		function GitView(props) {
			...
			/** The cwd-relative nested repo the user picked (null = the session
			*  cwd itself). The host resolves it under the session cwd, so a
			*  workspace root containing several repos can drill into each one. */
			const [selectedRepo, setSelectedRepo] = (0, react.useState)(null);
			/** Scope actually used for git calls: the selected repo when set. */
			const effScope = selectedRepo !== null ? { ...scope, repoPath: selectedRepo } : scope;
			const refresh = (0, react.useCallback)(async () => {
				setLoading(true);
				setError(null);
				try {
					const [statusResult, branchResult, logResult] = await Promise.all([
						api.gitStatus(effScope),
						api.gitBranch(effScope).catch(() => ({
							current: "",
							names: []
						})),
						api.gitLog(effScope, LOG_BATCH, 0).catch(() => [])
					]);
			...
			}, [scope.sessionId, scope.cwd, selectedRepo]);
			(0, react.useEffect)(() => {
				refresh();
			}, [refresh]);
			/** Dropping back to the session cwd when the panel's session moves. */
			(0, react.useEffect)(() => {
			...
			const loadMoreLog = async () => {
				...
				try {
					const next = await api.gitLog(effScope, LOG_BATCH, logEntries.length);
			...
			/** The diff tab for one changed file (one tab per repo+path+side; same
			*  id = focused). The tab record carries `repoPath` so the diff tab
			*  resolves against the same nested repo. */
			const openWorktreeDiff = (entry, staged) => {
				onOpenDiff({
					id: `diff:w:${staged ? "s" : "u"}:${selectedRepo !== null ? `${selectedRepo}/` : ""}${entry.path}`,
					type: "diff",
			...
			const stageEntry = async (entry, staged) => {
				setBusy(true);
				try {
					if (staged) await api.gitUnstage(effScope, entry.path);
					else await api.gitStage(effScope, entry.path);
					await refresh();
				} finally {
					setBusy(false);
				}
			...
			const stageAll = async (staged) => {
				setBusy(true);
				try {
					if (staged) await api.gitUnstage(effScope);
					else await api.gitStage(effScope);
					await refresh();
				} finally {
					setBusy(false);
				}
			...
			const commit = async () => {
				...
				setCommitError(null);
				try {
					await api.gitCommit(effScope, message);
					setCommitMsg("");
					await refresh();
				} catch (reason) {
			...
			const checkout = async (branch) => {
				...
				setCommitError(null);
				try {
					await api.gitCheckout(effScope, branch);
					await refresh();
				} catch (reason) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					!loading && status !== null && !status.isRepo && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: sidebar_module_css_default.gitPlaceholder,
						children: [t("notRepo"), (status.nestedRepos ?? []).length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: sidebar_module_css_default.gitEmpty,
								children: t("nestedRepos")
							}),
							(status.nestedRepos ?? []).map((repo) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
					...
					status !== null && status.isRepo && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						selectedRepo !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Menu, {
							...
							onSelect: (id) => {
								...
								if (target === null) return;
								setFileMenu(null);
								// Paths are relative to the SELECTED repo; prefix
								// the repo so editor-open and copy stay workspace-
								// unique while git ops keep the repo-relative form.
								const repoRel = selectedRepo !== null ? `${selectedRepo}/${target.entry.path}` : target.entry.path;
								if (id === "open") {
									onOpenFile(repoRel);
									return;
								}
								...
								if (id === "discard") {
									runConfirmed({
										...
										description: t("discardDesc", { path: repoRel }),
										confirmLabel: t("discard"),
										onConfirm: () => api.gitDiscard(effScope, target.entry.path)
									});
									return;
								}
								if (id === "relative") {
									copy(relativeTo(scope.cwd ?? "", repoRel));
									return;
								}
								if (id === "absolute") copy(repoRel);
							},
							portal: true,
							align: "start",
							...
							onSelect: (id) => {
								...
								if (id === "revert") {
									runConfirmed({
										...
										onConfirm: () => api.gitRevert(effScope, target.entry.hashFull)
									});
									return;
								}
								if (id === "cherryPick") runConfirmed({
									...
									onConfirm: () => api.gitCherryPick(effScope, target.entry.hashFull)
								});
							},
							portal: true,
							align: "start",
		...
		*/
		function DiffTab(props) {
			...
			(0, react.useEffect)(() => {
				let cancelled = false;
				// `diff.repoPath` (set when the git panel opened the tab from a
				// nested repo) redirects every call — diff, commit patch, and the
				// untracked fs.read — to that repo's scope.
				const scope = {
					sessionId,
					cwd,
					...diff.repoPath !== void 0 && diff.repoPath !== "" ? { repoPath: diff.repoPath } : {}
				};
				setLoading(true);
				setError(null);
				setData(null);
